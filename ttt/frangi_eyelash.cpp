...
void frangi2d_hessian_eyelash(const Mat &src, Mat &Dxx, Mat &Dxy, Mat &Dyy, float sigma){
    //construct Hessian kernels
//        int n_kern_x = 2*round(3*sigma) + 1;
    int n_kern_x = 2*round(2*sigma) + 1;
    ...
    int i=0, j=0;
    for (int x = -round(2*sigma); x <= round(2*sigma); x++){
        j=0;
        for (int y = -round(2*sigma); y <= round(2*sigma); y++){
...
//void frangi2d_createopts_eyelash(frangi2d_opts_t *opts){
...
void frangi2_eig2image_eyelash(const Mat &Dxx, const Mat &Dxy, const Mat &Dyy, Mat &lambda1, Mat &lambda2, Mat &Ix, Mat &Iy){
...
void frangi2d_eyelash(const Mat &src, Mat &maxVals, Mat &whatScale, Mat &outAngles, frangi2d_opts_t_eyelash opts){
    ...
    for (float sigma = opts.sigma_start; sigma <= opts.sigma_end; sigma += opts.sigma_step){
        //create 2D hessians
        Mat Dxx, Dyy, Dxy;
        frangi2d_hessian_eyelash(src, Dxx, Dxy, Dyy, sigma);
        ...
        frangi2_eig2image_eyelash(Dxx, Dxy, Dyy, lambda1, lambda2, Ix, Iy);
